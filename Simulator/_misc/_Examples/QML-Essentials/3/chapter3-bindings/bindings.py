...
"""
Property binding is a powerful feature of QML that allows values of different types to be synchronized automatically. 
It uses signals to notify and update other types values when property values are changed.
"""
...
class PieChart (QQuickPaintedItem):
    ...
    def color(self):
        return self._color # This ensures that colour of first permeates to 2nd. it is first called 
    ...
    def color(self, value):
        if value != self._color:
            self._color = value
            self.update()
            self.colorChanged.emit() # This signal ensures that the 2nd piechart colour is updated when the first one is.
